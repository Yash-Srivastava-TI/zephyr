...
/* Compile-time ABI compatibility check against Major Version */
BUILD_ASSERT(TI_PKTDMA_API_VERSION_MAJOR == 1,
	     "eth_ti_am62l_cpsw: incompatible TI PKTDMA API "
...
struct cpsw_priv {
	...
	/* TX DMA Channel completion interrupt */
	int tx_irq;

	/* RX DMA Channel completion interrupt */
	int rx_irq;
};
