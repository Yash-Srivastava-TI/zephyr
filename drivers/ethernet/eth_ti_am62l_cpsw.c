					 ...
					 uint32_t chan_id)
{
	...
	while (ti_pktdma_ring_pop(config->pktdma_dev, chan_id, &desc) == 0) {
		n++;
	}
	if (n > 0) {
...
static int cpsw_pktdma_setup(struct cpsw_priv *priv)
{
	...
	ret = ti_pktdma_chan_configure(config->pktdma_dev, config->tx_chan_id,
				       &tx_chan_cfg);
	if (ret < 0) {
		LOG_ERR("TX chan_configure failed: %d", ret);
		return ret;
	}
	...
	cpsw_drain_stale_completions(priv, config->tx_chan_id);

	ret = ti_pktdma_chan_configure(config->pktdma_dev, config->rx_chan_id,
				       &rx_chan_cfg);
	if (ret < 0) {
		LOG_ERR("RX chan_configure failed: %d", ret);
		return ret;
	}
	...
	cpsw_drain_stale_completions(priv, config->rx_chan_id);
	...
	for (int i = 0; i < CPSW_RX_DESC_NUM; i++) {
		...
		ret = ti_pktdma_ring_push(config->pktdma_dev, config->rx_chan_id,
...
static void cpsw_tx_irq_handler(const void *arg)
{
	...
	ti_pktdma_ring_irq_clear(config->pktdma_dev, config->tx_chan_id);
...
{
	uint32_t head_idx = (uint32_t)(((uintptr_t)*compl_phys -
...
static inline void cpsw_tx_handle_cmpl_batch(struct cpsw_priv *priv)
{
	...
	do {
		...
		while (count < CPSW_TX_BUDGET &&
		       ti_pktdma_ring_pop(config->pktdma_dev, config->tx_chan_id,
					  &compl_phys) == 0) {
...
static void cpsw_tx_cmpl_thread(void *arg1, void *arg2, void *arg3)
{
	...
	while (priv->ports_started != 0U) {
		...
		if (priv->ports_started != 0U) {
			ti_pktdma_ring_irq_arm(config->pktdma_dev,
					       config->tx_chan_id);
...
static void cpsw_rx_irq_handler(const void *arg)
{
	...
	ti_pktdma_ring_irq_clear(config->pktdma_dev, config->rx_chan_id);
					...
					struct net_buf *old_frag)
{
	...
	if (new_frag != NULL) {
		...
		ti_pktdma_ring_push(config->pktdma_dev, config->rx_chan_id,
				    *desc_phys);
	} else {
...
static inline void cpsw_rx_handle_batch(struct cpsw_priv *priv)
{
	...
	do {
		...
		while (count < CPSW_RX_BUDGET &&
		       ti_pktdma_ring_pop(config->pktdma_dev, config->rx_chan_id,
					  &desc_phys) == 0) {
...
static int cpsw_tx(const struct device *dev, struct net_pkt *pkt)
{
	...
	ret = ti_pktdma_ring_push(config->pktdma_dev, config->tx_chan_id,
				  (uint64_t)(uintptr_t)&dma->tx_descs[head_idx]);
	if (ret < 0) {
...
static int cpsw_start(const struct device *dev, struct net_if *iface)
{
	...
	if (priv->ports_started == 0U) {
		...
		ti_pktdma_ring_irq_arm(config->pktdma_dev, config->tx_chan_id);
		ti_pktdma_ring_irq_arm(config->pktdma_dev, config->rx_chan_id);
...
static int cpsw_init(const struct device *dev)
{
	...
	mac_lo = sys_read32(priv->mac_efuse_base);
	mac_hi = sys_read32(priv->mac_efuse_base + CPSW_MAC_LO_MAC_HI_OFFSET);

	/* Assign MAC Port 1's MAC Address */
	mac = priv->mac_addr[0];
	...
	/* Obtain Interrupts for TX and RX DMA Channels */
	priv->tx_irq = ti_pktdma_get_chan_irq(config->pktdma_dev,
					      config->tx_chan_id);
	priv->rx_irq = ti_pktdma_get_chan_irq(config->pktdma_dev,
					      config->rx_chan_id);
...
{											\
	.phy_dev = COND_CODE_1(DT_NODE_HAS_STATUS(CPSW_PHY_NODE(n, mdio_addr), okay),	\
