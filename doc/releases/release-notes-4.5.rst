...
* ``CONFIG_TFA_BUILD_FIP`` introduced to configure FIP (Firmware Image Package) generation.
  FIP generation is default disabled, but can be enabled by setting ``CONFIG_TFA_BUILD_FIP=y``
